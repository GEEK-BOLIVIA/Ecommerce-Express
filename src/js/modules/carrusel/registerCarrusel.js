...
export const RegisterCarrusel = {
    ...
    irAPaso2() {
        if (carruselActions.validarPaso1()) this.cambiarPaso(2);
    },

    cambiarTipo(tipo) {
        const cambio = carruselState.setTipo(tipo);
        if (cambio) this.updateUI();
    },
    ...
    agregarItemALista() {
        ...
        Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 })
            .fire({ icon: 'success', title: 'Cambios aplicados en la lista' });
    },
    ...
    cargarItemParaEditar(idx) {
        const itemsArray = Array.isArray(carruselState.items) ? carruselState.items : (carruselState.items.items || []);
        ...
        const itemMapeado = { ...item, titulo: valorTitulo, subtitulo: valorSubtitulo, link: valorLink, imagen_preview: valorMedia };
    ...
    reordenar(idx, direccion) {
        if (carruselState.reordenarItems(idx, direccion)) this.updateUI();
    },
    ...
    async actualizarOrdenAutomatico(nuevoSlug) {
        ...
        // ✅ Reemplazado: window.carruselController.obtenerSiguienteOrden() → carruselService.obtenerSiguienteOrden()
        const json = await carruselService.obtenerSiguienteOrden(nuevoSlug);
        const proximoOrden = json ?? 0;
